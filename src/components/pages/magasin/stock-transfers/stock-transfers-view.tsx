							...
							>
								...
								{transfer && (
									<Stack direction="row" spacing={1}>
										{permissions.can_edit && transfer.status !== 'validated' && (
											<Button
												variant="outlined"
												color="primary"
												size="small"
												startIcon={<EditIcon />}
												onClick={() => router.push(STOCK_TRANSFERS_EDIT(transfer.id))}
											>
												{t.common.edit}
											</Button>
										)}
										{permissions.can_create && transfer.status === 'draft' && (
											<Button
												variant="outlined"
												color="success"
												size="small"
										...
										{permissions.can_delete && transfer.status !== 'validated' && (
											<Button
												variant="outlined"
												color="error"
												size="small"
								...
								<Stack spacing={3}>
									...
									<DetailCard icon={<TransferIcon />} title={t.magasin.stockTransferDetails}>
										<InfoRow
											...
											value={transfer.target_store_name}
										/>
										<Divider />
										<InfoRow
											icon={<CalendarIcon />}
									...
									<DetailCard icon={<PersonIcon />} title={t.users.generalInfo}>
										...
										<InfoRow
											...
											value={formatDate(transfer.date_updated ?? null)}
										/>
									</DetailCard>
									<LineItemsCard
										icon={<InventoryIcon />}
