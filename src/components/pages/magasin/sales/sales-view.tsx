								...
								<Stack spacing={3}>
									...
									<DetailCard icon={<PaymentIcon />} title={t.magasin.paymentStatus}>
										<InfoRow
											...
											value={<StatusChip t={t} status={sale.payment_status} />}
										/>
										<Divider />
										<InfoRow
											icon={<NumbersIcon />}
											label={t.magasin.subtotal}
											value={`${formatNumber(sale.subtotal)} Dhs`}
										/>
										<Divider />
										<InfoRow
											icon={<NumbersIcon />}
											label={t.magasin.discountAmount}
											value={`${formatNumber(sale.discount_amount)} Dhs`}
										/>
										<Divider />
										<InfoRow
											icon={<NumbersIcon />}
											label={t.magasin.paidAmount}
											value={`${formatNumber(sale.paid_amount)} Dhs`}
										/>
										<Divider />
										<InfoRow
											icon={<NumbersIcon />}
											label={t.magasin.changeAmount}
											value={`${formatNumber(sale.change_amount)} Dhs`}
										/>
										<Divider />
										...
										columns={[
											{
												...
												render: (line) =>
													`${line.product_reference ?? line.product_barcode ?? '-'} - ${line.product_name}`,
											},
											{
												key: 'quantity',
											...
											},
											{
												...
												align: 'right',
												render: (line) => `${formatNumber(line.unit_price)} Dhs`,
											},
											{
												key: 'total',
