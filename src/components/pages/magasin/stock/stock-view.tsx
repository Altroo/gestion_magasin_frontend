									...
									>
										{permissions.can_edit && (
											<Button
												...
												size="small"
												startIcon={<EditIcon />}
												onClick={() => router.push(STOCK_EDIT(id, storeId))}
											>
												{t.common.edit}
											</Button>
										)}
									...
									<Card elevation={2} sx={{ borderRadius: 2 }}>
										<CardContent sx={{ p: 3 }}>
											...
											<InfoRow
												...
												value={stockBalance.product_reference ?? stockBalance.product_barcode}
											/>
											<Divider />
									...
									<Card elevation={2} sx={{ borderRadius: 2 }}>
										<CardContent sx={{ p: 3 }}>
											...
											<InfoRow
												...
												value={formatNumber(stockBalance.quantity)}
											/>
											<Divider />
											<InfoRow
												icon={<WarningIcon />}
												label={t.magasin.minimumStock}
												value={formatNumber(stockBalance.effective_min_stock)}
											/>
											<Divider />
											<InfoRow
												icon={<NumbersIcon />}
												label={t.magasin.averageCost}
												value={`${formatNumber(stockBalance.average_cost)} Dhs`}
											/>
											<Divider />
											<InfoRow
												icon={<WarningIcon />}
												label={t.magasin.lowStockStatus}
												value={stockBalance.is_low_stock ? t.magasin.lowStockReached : t.magasin.stockSufficient}
											/>
											<Divider />
											<InfoRow
												icon={<WarningIcon />}
												label={t.magasin.notifications}
												value={formatDateShort(stockBalance.low_stock_notified_at ?? null)}
											/>
										</CardContent>
									</Card>
								</Stack>
