							...
							>
								...
								{purchase && (
									<Stack direction="row" spacing={1}>
										{permissions.can_edit && purchase.status !== 'received' && (
											<Button
												variant="outlined"
												color="primary"
												size="small"
												startIcon={<EditIcon />}
												onClick={() => router.push(PURCHASES_EDIT(purchase.id))}
											>
												{t.common.edit}
											</Button>
										)}
										{permissions.can_create && purchase.status === 'draft' && (
											<Button
												variant="outlined"
												color="success"
												size="small"
											...
											>
												{t.magasin.receivePurchase}
											</Button>
										)}
										{permissions.can_delete && purchase.status !== 'received' && (
											<Button
												variant="outlined"
												color="error"
												size="small"
							...
							{isLoading ? (
								...
								<Stack spacing={3}>
									...
									<DetailCard icon={<StorefrontIcon />} title={t.magasin.purchaseDetails}>
										...
										<InfoRow
											icon={<NumbersIcon />}
											label={t.magasin.subtotal}
											value={`${formatNumber(purchase.subtotal)} Dhs`}
										/>
										<Divider />
										<InfoRow
											icon={<AttachFileIcon />}
									...
									<DetailCard icon={<PersonIcon />} title={t.users.generalInfo}>
										...
										<InfoRow
											...
											value={formatDate(purchase.date_updated ?? null)}
										/>
									</DetailCard>
									<LineItemsCard
										icon={<InventoryIcon />}
										...
										columns={[
											{
												...
												render: (line) =>
													`${line.product_reference ?? line.product_barcode ?? line.product} - ${line.product_name}`,
											},
											{
												key: 'quantity',
											...
											},
											{
												...
												align: 'right',
												render: (line) => `${formatNumber(line.unit_cost)} Dhs`,
											},
											{
												key: 'total',
												label: t.magasin.total,
												align: 'right',
												render: (line) => `${formatNumber(line.total)} Dhs`,
											},
										]}
									/>
								</Stack>
